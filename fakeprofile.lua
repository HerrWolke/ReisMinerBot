--[Onyx] ProfileChanger.lua by OurmineOGTv#6846
...
local OnyxGroupbox3 = gui.Groupbox(OnyxTab, "Profile Changer", 345, 335, 250);
...
local function profileChanger()
    if OnyxActivateProfile:GetValue() then
        ...
        local t1 = tostring(OnyxCMDfriendly:GetValue());
        local t2 = tostring(OnyxCMDteacher:GetValue());
        local t3 = tostring(OnyxCMDleader:GetValue());
        ...
        local fakeLvl = tostring(OnyxFakeLvl:GetValue())
        print(fakeLvl)
        ...
        panorama.RunScript([[
            if (PartyListAPI.GetXuidByIndex(0) != ]]..getTarget()..[[) {
                var machineName = "machine]]..getTarget()..[["

                if (]] .. activeCustomRank .. [[ == true) {
                    if (]] .. tostring(getRankToChange() ~= "Unranked") .. [[ == true) {
                        updateMessage += "Update/Members/" + machineName + "/player0/game/ranking " + ]] .. getRankToChange() .. [[ + " ";
                    }
                }

                if (]] .. tostring(OnyxFakeLvl:GetValue() ~= "") .. [[ == true) {
                    updateMessage += "Update/Members/" + machineName + "/player0/game/level " + ]] .. fakeLvl .. [[ + " ";
                }

               if (]] .. fakePrimeActive .. [[ == true) {
                    updateMessage += "Update/Members/" + machineName + "/player0/game/prime " + ]] .. (fakePrime and "1" or "0") .. [[ + " ";
               }

                updateMessage += "Update/Members/" + machineName + "/player0/game/commends " + "[f]]..t1..[[][t]]..t2..[[][l]]..t3..[[] ";
            }
...
gui.Button(OnyxGroupbox3, "Force Reload", profileChanger)
